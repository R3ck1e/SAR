...
Для пилота удобнее распространять готовый `.exe` для Windows или `.app` для macOS. Docker имеет смысл использовать для серверной части обработки.

## Что описывать в ВКР

В технологической части можно описать разделение системы на клиентскую часть и модуль обработки. Клиентская часть работает на наземной станции, позволяет оператору выбрать видео, задать параметры и просмотреть результаты. Серверная часть выполняет извлечение кадров, запуск YOLO, формирование JSON-метаданных и аннотированных кадров.

Такое разделение позволяет использовать удаленный сервер при наличии сети и локальный режим при ее отсутствии. Технические файлы скрыты от пользователя интерфейсом, поэтому оператор работает не с папками и JSON напрямую, а с заданиями, кадрами и найденными объектами.
